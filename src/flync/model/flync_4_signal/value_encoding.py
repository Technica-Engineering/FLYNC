...
class TextEntry(FLYNCBaseModel):
    ...
    Omit ``to_value`` for single-value mappings; it defaults to ``from_value``.
    ...
    to_value : int
        Inclusive upper bound of the raw value range.  Optional; defaults to
        ``from_value`` for a single-value mapping.
    label : str
