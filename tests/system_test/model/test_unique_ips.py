...
def _compute_node_iface_config(workspace_root, node):
    return (
        workspace_root
        / "ecus"
        / "eth_ecu"
...
def _set_viface_ip(iface_config, viface, old_ip, new_ip):
    ...
    with patch_yaml(iface_config) as config:
        addresses = entry_named(config["virtual_interfaces"], viface)["addresses"]
...
def test_unique_ips_duplicate_ipv4_across_ecus(tmpdir, example_experimental_workspace_path):
    ...
    _set_viface_ip(_compute_node_iface_config(destination_folder, "eth_ecu_node2"), "eth_ecu_node2_viface1", "10.0.50.7", "10.0.50.1")
...
def test_unique_ips_duplicate_ipv4_within_same_ecu(tmpdir, example_experimental_workspace_path):
    ...
    destination_folder = _load_example(tmpdir, example_experimental_workspace_path)
    # eth_ecu_node1 already declares 10.0.40.7 in vlan 40.  Set the second
    # compute node's address to the same IP to trigger an in-ECU clash.
    _set_viface_ip(_compute_node_iface_config(destination_folder, "eth_ecu_node2"), "eth_ecu_node2_viface1", "10.0.50.7", "10.0.40.7")
...
def test_unique_ips_dynamic_ipv4_zero_address_is_allowed(tmpdir, example_experimental_workspace_path):
    ...
    destination_folder = _load_example(tmpdir, example_experimental_workspace_path)
    # Both node IPs become 0.0.0.0; even though they collide, no warning must be raised.
    _set_viface_ip(_compute_node_iface_config(destination_folder, "eth_ecu_node1"), "eth_ecu_node1_viface1", "10.0.40.7", "0.0.0.0")
    _set_viface_ip(_compute_node_iface_config(destination_folder, "eth_ecu_node2"), "eth_ecu_node2_viface1", "10.0.50.7", "0.0.0.0")
...
def test_unique_ips_multiple_duplicates_each_emit_a_warning(tmpdir, example_experimental_workspace_path):
    ...
    _set_viface_ip(_compute_node_iface_config(destination_folder, "eth_ecu_node1"), "eth_ecu_node1_viface1", "10.0.40.7", "10.0.40.1")
    _set_viface_ip(_compute_node_iface_config(destination_folder, "eth_ecu_node2"), "eth_ecu_node2_viface1", "10.0.50.7", "10.0.50.1")
