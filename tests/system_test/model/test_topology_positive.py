...
def valid_ecu_with_switch():
    ...
    ipv4_ecu1 = IPv4AddressEndpoint(address=IPvAnyAddress('192.0.2.1'),ipv4netmask=IPvAnyAddress('192.0.0.0'))
    virtual_iface_ecu1 = VirtualControllerInterface(name="virtual",vlanid=55,addresses=[ipv4_ecu1])
    ctrl_iface_ecu1 = ControllerInterface(name="control1",mac_address=MacAddress("00:00:5e:00:53:01"),virtual_interfaces=[virtual_iface_ecu1],mii_config=MII(type="mii", speed=100, mode="phy"))
    ...
    switch_to_controller_cnx1 = SwitchPortToControllerInterface(type="switch_port_to_controller_interface",id="2",switch_port="sw_port1",controller_interface="control1")

    # --- Internal links ---
    internal_topology_ecu1 = InternalTopology(connections=[InternalConnectionUnion(root=ecu_to_controller_cnx1),InternalConnectionUnion(switch_to_controller_cnx1)])

    # --- ECU Metadata ---
    ecu_metadata = ECUMetadata(type="ecu",author="test_team",compatible_flync_version=system_version)

    # --- Create the ECU ---
    ecu1 = ECU(name="ecu1",ports=[port_ecu1],controllers=[controller_ecu1],topology=internal_topology_ecu1,ecu_metadata=ecu_metadata,switches=[switch1])
...
def valid_iface_to_iface():
    ...
    ipv4_ecu2 = IPv4AddressEndpoint(address=IPvAnyAddress('192.0.2.2'),ipv4netmask=IPvAnyAddress('192.0.0.0'))
    virtual_iface_ecu2 = VirtualControllerInterface(name="virtual2",vlanid=55,addresses=[ipv4_ecu2])
    ctrl_iface_ecu2 = ControllerInterface(name="control2",mac_address=MacAddress("00:00:5e:00:53:02"),virtual_interfaces=[virtual_iface_ecu2],mii_config=MII(type="mii", speed=100 ,mode="mac"))
    ...
    controller_to_controller_cnx1 = ControllerInterfaceToControllerInterface(type="controller_interface_to_controller_interface",id='2',controller_interface1="control1",controller_interface2="control2")

    # --- Internal links ---
    internal_topology_ecu1 = InternalTopology(connections=[InternalConnectionUnion(root=ecu_to_controller_cnx1),InternalConnectionUnion(root=controller_to_controller_cnx1)])

    # --- ECU Metadata ---
    ecu_metadata = ECUMetadata(type="ecu",author="test_team",compatible_flync_version=system_version)

    # --- Create the ECU ---
    ecu1 = ECU(name="ecu1",ports=[port_ecu1],controllers=[controller_ecu1,controller_ecu2],topology=internal_topology_ecu1,ecu_metadata=ecu_metadata)
...
def test_flync_model(tmpdir, valid_model_func):
    ...
    workspace = FLYNCWorkspace.load_model(flync_model=initial_model,workspace_name="TestWorkspace",file_path=workspace_path)
