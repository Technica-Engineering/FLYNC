...
class TestEnumEntries:
    ...
    def test_duplicate_value_raises(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Enum(
                name="MyEnum",
                entries=[
                    EnumEntry(value=1, name="first"),
                    EnumEntry(value=1, name="second"),
                ],
            )
    ...
    def test_value_above_base_type_range_raises(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Enum(name="MyEnum", entries=[EnumEntry(value=256, name="too_big")])
    ...
    def test_value_below_int8_range_raises(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Enum(name="MyEnum", base_type=Int8(), entries=[EnumEntry(value=-129, name="too_small")])
    ...
    def test_invalid_base_type_reports_cleanly(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Enum(name="MyEnum", base_type={"type": "nonsense"}, entries=[EnumEntry(value=256, name="too_big")])
