...
def test_unknown_measurement_point_type_is_rejected():
    ...
    with pytest.raises(ValidationError) as exc_info:
        TypeAdapter(list[MeasurementPointType]).validate_python([data])
    assert_single_error(exc_info, None, "nope")


def test_rejects_duplicate_payload_types():
    with pytest.raises(ValidationError) as exc_info:
        TypeAdapter(list[MeasurementPointType]).validate_python([_points()[0] | {"payload_types": ["can", "can"]}])
...
def test_rejects_port_count_outside_one_or_two(num_ports):
    ...
    with pytest.raises(ValidationError) as exc_info:
        TypeAdapter(list[MeasurementPointType]).validate_python([{"name": "p2p", "type": "ethernet_ports", "ports": ports}])
...
def test_interface_id_above_32_bit_is_rejected():
    with pytest.raises(ValidationError) as exc_info:
        TypeAdapter(list[MeasurementPointType]).validate_python([_points()[0] | {"interface_id": 2**32}])
